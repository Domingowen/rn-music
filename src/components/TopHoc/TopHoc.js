...
// import Request from 'src/common/request';
const baseUrl = "https://233333333.itooi.cn/";
...
const TopHoc = Wrapper => {
    class TopHocWrapper extends Component {
        ...
        init() {}
        async add(item) {
            // 这个只有在在线搜索, 分类歌单，歌手详情 才会去调用的接口，本地的，缓存不调用该接口
            console.log(item);
            // let songUrl = null;
            // songUrl = await this.getMusicUrl(item.id);
            // console.log(songUrl);
            Toast.show({
        ...
        async download(item, local) {
            // 下载到本地， 最近播放，我的喜欢都会有下载到本地的情况
            Toast.show({

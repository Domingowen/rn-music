    ...
    return (
        props.player && <View style={{
            ...
            <View style={{
                ...
                <View style={{borderRadius: 50, width: 50, height: 50, overflow: 'hidden'}}>
                    {props.player && <Image
                ...
                <View style={{paddingLeft: 10}}>
                    <Text>{props.player && props.player.title}</Text>
                    <Text>{props.player && props.player.artist}</Text>
                </View>
            </View>
            <View>
                <TouchableOpacity onPress={() => settingMusicLoop()}>
                    {
                        loop ? // true single cycle
                                // false list loop
                            <Icon type={'MaterialIcons'} name={'repeat-one'}
            ...
            <View>
                <TouchableOpacity style={{position: 'relative', justifyContent: 'center', alignItems: 'center'}}
                                  onPress={() => playStatusHandle()} activeOpacity={0.8}>
                    <Progress.Circle size={40} progress={playProgress} indeterminate={false}
                                     borderWidth={1} borderColor={'#31c27c'} color={'#31c27c'}>
                    </Progress.Circle>
                    {playStatus ?
                        <Icon type={'AntDesign'} name={'pause'}
                              style={{color: '#31c27c', fontSize: 24, position: 'absolute', top: 8}}/> :

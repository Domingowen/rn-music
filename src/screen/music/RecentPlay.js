...
function MusicRecentPlay (props) {
    ...
    function renderItems ({item, index}) {
        // console.log(item);
        return <ListItem onPress={() => play(item)}>
            <Body>
            <Text style={{color: item.play ? '#31c27c': '#000'}}>{item.title}</Text>
            <Text note style={{color: item.play ? '#31c27c': '#000'}}>
                {item.download && <Icon type={'AntDesign'} name={'checkcircle'} style={{color: '#31c27c', fontSize: 12}}/>}
                {item.artist} {item.album}
            </Text>
            </Body>
            <Right style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end'}}>
                {!item.download && <Icon type={"AntDesign"} name={'clouddownload'} style={{color: '#31c27c', fontSize: 35}} onPress={() => download(item)}/>}
            </Right>
        </ListItem>
    }
    function emptyItem () {
        return <View style={{height: 400, alignItems: 'center', justifyContent: 'center'}}>
            <Text>最近没有播放过音乐</Text>
        </View>
    }
    return <Container>
...
export default TopHoc(connect(mapStateToProps, mapDispatchToProps)(MusicRecentPlay));
...
export default function Find(props) {
    ...
    useEffect(() => {
        const init = async () => {
            /*热门歌单*/
            await Request('tencent/songList/hot?cat=全部&pageSize=11&page=0').then(res => {
                setData(prevState => ({
                    ...prevState,
                    hot: res.data.list,
                }));
            ...
            /*热门歌手*/
            await Request('tencent/artist/list?sexId=-100&areaId=-100&genre=-100&index=-100&page=0&pageSize=30').then(res => {
                setData(prevState => ({
                    ...prevState,
                    singer: res.data,
                }));
    ...
    let singerDetail = (item) => { //歌手详情
        props.navigation.navigate('SingerDetail', {
            item: item
        })
    };

    let classifyDetail = (item) => { // 歌单详情
        props.navigation.navigate('ClassifyDetail', {
            item: item
        })
    };
    let renderHotItem = () => { // 热门歌单
        return data.hot && <View>
            <View>
                <Text
                    style={{textAlign: 'center', fontSize: 20, paddingBottom: 20, letterSpacing: 5}}>为你推荐的歌单</Text>
            </View>
                ...
                {data.hot.map((val, index) => {
                    ...
                    >
                        <View style={styles.hotItem}>
                            ...
                            />
                            <Text style={{fontSize: 12, paddingTop: 10}}>{val.dissname}</Text>
                        </View>
                    </TouchableOpacity>
                })}
            </View>
    ...
    let renderSingerItem = () => { // 热门歌手
        return data.singer && <View>
            <View>
                <Text
                    style={{textAlign: 'center', fontSize: 20, paddingBottom: 20, letterSpacing: 5}}>你会喜欢的歌手</Text>
            </View>
            <View style={{flexWrap: 'wrap', flexDirection: 'row', paddingBottom: 20}}>
                {data.singer.slice(0, 12).map((val, index) => {
                    ...
                    return <TouchableOpacity activeOpacity={0.8}
                        ...
                        <View style={styles.hotItem}>
                            ...
                            />
                            <Text style={{fontSize: 12, paddingTop: 10}}>{val.singer_name}</Text>
                        </View>
                    </TouchableOpacity>
    ...
    let renderHeader = () => { // 头部
        return <View
            ...
            {topHeaderIconArr.map((val, index) => {
                return <View key={index} style={{width: '33%'}}>
                    <TouchableOpacity style={{alignItems: 'center', width: '100%'}} activeOpacity={0.6}
                        ...
                        <SvgUri
                            ...
                            // fill={'#31c27c'}
                        />
                        <Text style={{paddingTop: 5}}>{val.title}</Text>
                    </TouchableOpacity>
                </View>
            })}
        </View>
